...
Where $p_{l,j}^{t-1}$ is the probability of state $j$ at time  $t-1$ given that the state $l$ at time $0$  ~\cite{anderson1957statistical}. We are interested in the variances of $\mathrm{Var}(\hat{p}_{i,j} - {p}_{i,j})$ to study the quality of the estimates, which can be computed as follows:
...
as\ N \xrightarrow{} \infty\\
where\ N = k \times n.\\
and\ \mathrm{Var} (\hat{\pi}_{i,j} - {p}_{i,j}) = \frac {\sigma^{2}_{mle_N}}{N} =  \frac {\sigma^{2}_{mle_n}}{k \times n}
\end{aligned}
\end{equation}



That is, since $N > n$ combining $k$ sequences, the variances  of our method estimates $\mathrm{Var} (\hat{\pi}_{i,j} - {p}_{i,j})$  are smaller than the estimates of MLE over an isolated sequence $\mathrm{Var} (\hat{p}_{i,j} - {p}_{i,j})$. Thus, by Chebyshev's inequality \cite{feller1968introduction} we have the guarantee  ..
...
To summarize, our approach aggregating the MLE estimates over $k$ sequences speeds up the convergence  to reach the true transition probabilities as result of the  smaller variances.
